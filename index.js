...
class Action {
    ...
    _pushPackage(version, name) {
        ...
        this._executeInProcess(`dotnet pack -p:PackageVersion=${this.version} -p:PackageId=${this.packageName} ${this.includeSymbols ? "--include-symbols -p:SymbolPackageFormat=snupkg" : ""} --no-build -c Release ${this.projectFile} -o .`)
